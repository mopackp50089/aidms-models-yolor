...
from torch.nn.parallel import DistributedDataParallel as DDP #Harry: 多機多卡分佈訓練
...
logger = logging.getLogger(__name__) #Harry:以module的名稱建立一個logger物件,共五個Level:(1)Debug(2)Info(3)Warning(4)error(5)critical

#harry
from torchsummary import summary
#########################
#aidms hyperparameters
import sys
...
try:
    import wandb #Harry: 用來記錄監控超參數的package
except ImportError:
    wandb = None
...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    wdir.mkdir(parents=True, exist_ok=True)  # make dir #Harry import pathlib
    last = wdir / 'last.pt'
    best = wdir / 'best.pt'
    ...
    ###########################################
    #Harry 
    #change train_path,test_path,nc,names
    with open(opt.dataset_yaml) as f:
        data_dict = yaml.load(f, Loader=yaml.FullLoader)  # data dict
    with torch_distributed_zero_first(rank): #Harry 使用torch.distributed.barrier() 來同步所有進程
        check_dataset(data_dict)  # check
    train_path = data_dict['train']
    test_path = data_dict['val']
    nc, names = (1, ['item']) if opt.single_cls else (int(data_dict['nc']), data_dict['names'])  # number classes, names
    assert len(names) == nc, '%g names found for nc=%g dataset in %s' % (len(names), nc, opt.dataset_yaml)  # check
    ###########################################

    # Model
    pretrained = weights.endswith('.pt') #Harry 檢查str後綴字 return Ture or False
    pretrained2 = weights.endswith('.pth')
    
    if pretrained or pretrained2:
        with torch_distributed_zero_first(rank):#Harry 使用torch.distributed.barrier() 來同步所有進程
    ...
    assert mlc < nc, 'Label class %g exceeds nc=%g in %s. Possible class labels are 0-%g' % (mlc, nc, opt.dataset_yaml, nc - 1)
    ...
    assert start_epoch < epochs, f"epochs must > {start_epoch}"
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if rank in [-1, 0]:
                ...
                # print('-----------------------------------------------------------------------------')
                # mem_allocated = '%.3gG' % (torch.cuda.memory_allocated() / 1E9 if torch.cuda.is_available() else 0)  # (GB)
                # print(f"memory-memory_allocated :{mem_allocated} ")
                # print('-----------------------------------------------------------------------------')
                # mem_max_allocated = '%.3gG' % (torch.cuda.max_memory_allocated() / 1E9 if torch.cuda.is_available() else 0)  # (GB)
                # print(f"max_memory-memory_allocated :{mem_max_allocated} ")
                # print('-----------------------------------------------------------------------------')
                # mem_cached = '%.3gG' % (torch.cuda.memory_cached() / 1E9 if torch.cuda.is_available() else 0)  # (GB)
                # print(f"memory_cached :{mem_cached} ")
                # print('-----------------------------------------------------------------------------')
        ...
        if rank in [-1, 0]:
            ...
            if not opt.notest or final_epoch:  # Calculate mAP
                ...
                if epoch >= 3:
                #if epoch%100 == 0:
                    results, maps, times = test.test(opt.dataset_yaml,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='yolor_dataset/yolor_dataset_p6.cfg', help='model.yaml path')
    parser.add_argument('--data', type=str, default='yolor_dataset/yolor_dataset.yaml', help='data.yaml path')
    ...
    parser.add_argument('--evolve', action='store_true', help='evolve hyperparameters')#Harry : 利用GA優化超參數
    parser.add_argument('--bucket', type=str, default='', help='gsutil bucket')#Harry: 雲端storage
    ...
    #set yolor hyperparameters to aidms hyperparameters 
    opt.cfg = model_parameters.save_backbone_cfg
    opt.dataset_yaml = model_parameters.save_dataset_yaml
    opt.weights = model_parameters.save_weight_path
    opt.batch_size = model_parameters['training', 'default_para', 'batch'][0]
    opt.epochs = model_parameters['training', 'default_para', 'epoch'][0]
    opt.img_size[0] = model_parameters['training', 'advanced_para', 'image_size', 'Height'][0]
    opt.img_size[1] = model_parameters['training', 'advanced_para', 'image_size', 'Width'][0]
    opt.workers = model_parameters['training', 'advanced_para', 'workers'][0]
    ...
    if opt.resume:  # resume an interrupted run
        ...
        opt.dataset_yaml, opt.cfg, opt.hyp = check_file(opt.dataset_yaml), check_file(opt.cfg), check_file(opt.hyp)  # check files
        ...
        opt.save_dir = str(save_dir)  # increment run
    ...
    if not opt.evolve:
        ...
        if opt.global_rank in [-1, 0]:
            logger.info(f'Start Tensorboard with "tensorboard --logdir {model_parameters.save_tensorboard}", view at http://localhost:6006/')
            #harry aidmsf
            tb_writer = SummaryWriter(model_parameters.save_tensorboard)
