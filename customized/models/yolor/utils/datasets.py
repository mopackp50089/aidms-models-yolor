...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
    ...
    #harry path=/workspace/models/yolor/gear_data_structure/images/train_gear/
...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, auto_size=32):
        ...
        if any(videos):
            ...
            #harry 3. check wherher or not vedio
            self.cap = None
    ...
    def __next__(self):
        if self.count == self.nf:
            raise StopIteration
        #harry 4.iteration inference image absolute path
        ...
        if self.video_flag[self.count]:
            ...
            print('line212',type(img0),img0.shape)
            assert img0 is not None, 'Image Not Found ' + path
            print('image %g/%g %s: ' % (self.count, self.nf, path), end='')

        # Padded resize
        #harry 6. preprocessing img0 shape: (512, 773, 3) self.auto_size=64
        print('line218',self.auto_size)
        img = letterbox(img0, new_shape=self.img_size, auto_size=self.auto_size)[0]
        # Convert
        print('line219',img.shape)
        img = img[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
        print('line220',img.shape)
        img = np.ascontiguousarray(img)
        print('line221', img.shape)
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        #self.label_files = /workspace/models/yolor/gear_data_structure/labels/train_gear/NG-00028_A_S.txt',...
        #Path(self.label_files[0]).parent = /workspace/models/yolor/gear_data_structure/labels/train_gear
        cache_path = str(Path(self.label_files[0]).parent) + '.cache3'  # cached labels
        #cache_path = /workspace/models/yolor/gear_data_structure/labels/train_gear.cache3
        if os.path.isfile(cache_path):
            cache = torch.load(cache_path)  # load
            #cache = gear_data_structure/images/train_gear/NG-00028_A_S.png : label array
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        # Load image
        #raise Exception(f"{self.img_files}")
